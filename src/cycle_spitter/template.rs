...
pub fn parse_template(template_content: &str, number_re: &Regex) -> Result<Vec<TemplateSection>, Box<dyn Error>> {
...
mod tests {
    ...
    fn test_parse_template_instruction_with_cycles() {
        let content = r#"
            ...
            dcb.w 2, $4e71
        "#;
        let number_re = Regex::new(r"move\.w\s*#\$([0-9a-fA-F]+)").unwrap();
        let sections = parse_template(content, &number_re).unwrap();

        assert_eq!(sections.len(), 1);
        assert_eq!(sections[0].nop_cycles, 8); // 2 * 4
        assert_eq!(sections[0].injection_code.len(), 1);
        assert_eq!(sections[0].injection_code[0].0, "move.w #$1234, D0 ; Move Instruction");
    ...
    fn test_parse_template_multiple_sections() {
        let content = r#"
            move.w #$5678, D1
            dcb.w 4, $4e71
            move.w #$9ABC, D2 ; Label for section
            dcb.w 6, $4e71
        "#;
        let number_re = Regex::new(r"move\.w\s*#\$([0-9a-fA-F]+)").unwrap();
        let sections = parse_template(content, &number_re).unwrap();
        ...
        // Section 1
        assert_eq!(sections[0].nop_cycles, 16); // 4 * 4
        assert_eq!(sections[0].injection_code.len(), 1);
        assert_eq!(sections[0].injection_code[0].0, "move.w #$5678, D1");
        assert_eq!(sections[0].label, "Section 1");

        // Section 2
        assert_eq!(sections[1].nop_cycles, 24); // 6 * 4
        assert_eq!(sections[1].injection_code.len(), 1);
        assert_eq!(sections[1].injection_code[0].0, "move.w #$9ABC, D2 ; Label for section");
    ...
    fn test_parse_template_ignore_empty_lines() {
        ...
        assert_eq!(sections.len(), 1);
        assert_eq!(sections[0].nop_cycles, 4); // 1 * 4
        assert_eq!(sections[0].injection_code.len(), 1);
        assert_eq!(sections[0].injection_code[0].0, "move.w #$ABCD, D3 ; Some work");
    ...
    fn test_parse_template_with_inline_comments() {
        ...
        assert_eq!(sections.len(), 1);
        assert_eq!(sections[0].nop_cycles, 28); // 7 * 4
        assert_eq!(sections[0].injection_code.len(), 1);
        assert_eq!(sections[0].injection_code[0].0, "move.w #$AAAA, D4 ; Inline comment");
        assert_eq!(sections[0].label, "Inline comment");
    }

    #[test]
    fn test_parse_template_no_valid_cycles() {
        let content = r#"
            nop
            nop
        "#;
        ...
        assert_eq!(sections.len(), 0); // No valid sections
    }
}